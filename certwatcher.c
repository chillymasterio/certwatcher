...
static void print_cert_normal(const cert_info_t *info, int warn_days, int verbose) {
    ...
    {
        int total_days = info->days_since_issue + info->days_left;
        int pct = total_days > 0 ? (int)(100.0 * info->days_since_issue / total_days) : 0;
        printf("  Lifetime:    %d days total, %d%% elapsed\n", total_days, pct);
    }
...
static void usage(const char *prog) {
    fprintf(stderr,
        ...
        "  --san-list      All Subject Alternative Names\n"
        "  --expiry-date   Raw expiry date\n"
        "  --days-only     Days remaining only\n"
        "  --timing-only   Connection timing breakdown\n"
        "  --ocsp-only     OCSP stapling status\n"
        "\n"
        "Filters:\n"
        "  --self-signed-only  Show only self-signed certs\n"
        "  --weak          Show only weak certs (old TLS, small keys)\n"
        "\n"
    ...
    if (parallel && nhost > 1) {
        ...
        for (i = 0; i < nhost; i++) {
            ...
            for (attempt = 0; attempt <= retries; attempt++) {
                rc = fetch_cert(parsed_hosts[i], parsed_ports[i],
